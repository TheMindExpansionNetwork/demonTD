...
def wire_callbacks(demon):
    ...
    # Script CHOP callbacks. Unlike WebSocket/Timer DATs, TD's Script CHOP
    # does NOT reliably honor `par.callbacks` pointing at a sibling DAT —
    # the cook callback only fires when the code lives in a CHILD DAT of
    # the Script CHOP itself. We create that child DAT here and route its
    # onCook into the extension.
    for name, ext_method in (("script_send", "OnCookSend"),
                             ("audio_out", "OnCookRecv")):
        s = demon.op(name)
        if s is None:
            continue
        child = s.op("script")
        if child is None:
            child = s.create(textDAT, "script")
            try:
                child.nodeX, child.nodeY = 0, -100
            except Exception:
                pass
        child.text = (
            "# auto-generated by build_tox.py — DO NOT EDIT BY HAND.\n"
            "# Lives inside the Script CHOP because TD ignores par.callbacks\n"
            "# pointing at a sibling DAT for Script CHOP cook routing.\n"
            ...
            "def onCook(scriptOp):\n"
            "    try:\n"
            f"        parent().ext.DemonExt.{ext_method}(scriptOp)\n"
            "    except Exception as e:\n"
            f"        print(f\"[{name} onCook] {{e}}\")\n"
        )
        # Point the Script CHOP at the child. Relative name 'script' resolves
        # to a CHILD of audio_out (not a sibling).
        try:
            s.par.callbacks = "script"
        except Exception as e:
            print(f"!! {name}.par.callbacks: {e}")
